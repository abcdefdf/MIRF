...
def increase_dataset(gen, generated_num, epoch_num):
    ...
    for s in samples:
        ...
        str1 = ''.join(new_int)
        new_str.append(str1)
    feature = extract_all_features(new_str)
    label = np.array([[1] for i in range(len(feature))])
    feature = np.column_stack((feature, label))
    np.savetxt('exp' + str(epoch_num) + '.csv', feature, delimiter=',')
...
def train_discriminator():
    ...
    clf = randomforest.RandomForestClassifier(n_estimators=1, subsample=1.0, random_state=66)
...
def main(model_name, use, cuda):
    ...
    for epoch in range(PRE_EPOCH_NUM):
        ...
        print('Pretrain Epoch [%d] Model Loss: %f' % (epoch, loss))
    ...
    for total_batch in range(TOTAL_BATCH):
        ...
        # ***********************************************************************************************************#
        # 检测器增量更新
        df = increase_dataset(generator, 4096, total_batch)
        increase_data = df[df['label'].isin([0, 1])].sample(frac=1, random_state=66).reset_index(drop=True)
        for _, row in tqdm(increase_data.iterrows()):
            discriminator.increase_sample(row[:-1], row[-1:])

        t_auc, dis_loss_i, gen_loss_i, reward_i = AUC(generator, discriminator, model_name, 4096)
        print('Epoch_increase[%d]: auc is %f gen_loss is %f dis_loss is %f' % (total_batch, t_auc, gen_loss_i, dis_loss_i))
        print(auc, dis_loss, gen_loss)
        auc.append(t_auc)
        dis_loss.append(dis_loss_i)
        gen_loss.append(gen_loss_i)
